...
void syscalls_myplugin_base::print_sysret(drakvuf_t drakvuf, drakvuf_trap_info_t* info, int nr, const char* extra_info)
{
...
std::string syscalls_myplugin_base::parse_argument(drakvuf_t drakvuf, drakvuf_trap_info_t* info, const arg_t& arg, addr_t val)
{
    char* cstr = nullptr;
...
uint64_t syscalls_myplugin_base::mask_value(const arg_t& arg, uint64_t val)
{
    switch (arg.type)
    {
        case BYTE:
...
uint64_t syscalls_myplugin_base::transform_value(drakvuf_t drakvuf, drakvuf_trap_info_t* info, const arg_t& arg, uint64_t val)
{
...
bool syscalls_myplugin_base::read_syscalls_filter(const char* filter_file)
{
...
syscalls_myplugin_base::syscalls_myplugin_base(drakvuf_t drakvuf, const syscalls_myplugin_config* config, output_format_t output) : pluginex(drakvuf, output)
{
...
syscalls_myplug::syscalls_myplug(drakvuf_t drakvuf, const syscalls_myplugin_config* config, output_format_t output) : pluginex(drakvuf, output)
{
    os_t os = drakvuf_get_os_type(drakvuf);
    if (os == VMI_OS_WINDOWS)
        this->_win_syscalls = std::make_unique<win_syscalls_myplug>(drakvuf, config, output);
