...
bool win_syscalls_myplug::trap_syscall_table_entries(drakvuf_t drakvuf, vmi_instance_t vmi, addr_t cr3, bool ntos, addr_t base, std::array<addr_t, 2> _sst, json_object* json)
{
    ...
    // READ THE ADDRESSES OF SYSTEM SERVICE TABLE INTO variable "table"
    ACCESS_CONTEXT(ctx);
    ctx.translate_mechanism = VMI_TM_PROCESS_DTB;
    ctx.dtb = cr3;
    ctx.addr = _sst[0];
    ...
    for ( addr_t syscall_num = 0; syscall_num < _sst[1]; syscall_num++ )
    {
        ...
        if ( !this->is32bit )
        {
            ...
            syscall_va = _sst[0] + offset; // find the virtual address of syscall
        }
        else
            syscall_va = table[syscall_num];

        addr_t rva = syscall_va - base; // get the RVA for the syscall so you can find its symbol
        if ( this->is32bit )
        ...
        if ( symbols )
        {
            ...
            {
                if ( symbols->symbols[z].rva == rva ) //  match the function found in ssdt with the one located in symbols(if there is one) 
                {
                    symbol = &symbols->symbols[z]; // get the symbol for syscall
    ...
    , win32k_profile{ config->win32k_profile ?: "" }
{
    ...
    if ( !this->is32bit )
    {
        system_service_table_x64 _sst[2] = {}; // KeServiceDescriptorTableShadow is an extension to KeServiceDescriptorTable, it contains Windows Native Api SST as well as win32k SST
        ...
        this->sst[0][0] = _sst[0].ServiceTable; // ServiceTable: points to an array of virtual addresses – the SSDT (System Service Dispatch Table), where each entry further points to a kernel routine.
        this->sst[0][1] = _sst[0].ServiceLimit; // ServiceLimit: number of entries in the SSDT table.
    ...
    if (!drakvuf_get_kernel_struct_member_rva(drakvuf, "_RTL_USER_PROCESS_PARAMETERS", "ImagePathName", &this->image_path_name)) // get offset for image_path_name attr in kernel struct
    {
        PRINT_DEBUG("[SYSCALLS] Failed to get ImagePathName from _RTL_USER_PROCESS_PARAMETERS\n");
        throw -1;
    }

    if (!trap_syscall_table_entries(drakvuf, vmi, dtb, true, this->kernel_base, this->sst[0], vmi_get_kernel_json(vmi))) // trap Windows Native Api calls
    {
